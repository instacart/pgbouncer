...
static bool sbuf_process_pending(SBuf *sbuf)
{
	...
	while (1) {
		...
		case ACT_SEND:
			log_info("ACT_SEND - avail: %d, sbuf->pkt_remain: %d", avail, sbuf->pkt_remain);
			// log_info("res = %d", res);
			char hex[8];
			log_info("hex: %p, size: %lu", hex, sizeof(hex));

			DumpHex(&sbuf->current_pkt->data, 64);
			
			/*
			log_info("current_pkt: %p, type: %c, len: %d", sbuf->current_pkt, sbuf->current_pkt->type, sbuf->current_pkt->len);
			...
			*/
			...
			// struct MBuf *buf, const void *ptr, unsigned len)
			// mbuf_write(&sbuf->current_pkt->data, &io->buf[io->done_pos], avail);

			iobuf_tag_send(io, avail);

			// sbuf_call_proto(sbuf, SBUF_EV_READ_MORE);
			break;
		case ACT_CALL:
